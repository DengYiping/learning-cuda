...
__global__ void sram_matmul(float* A, float* B, float* C, int M, int N, int K) {
    ...
    __shared__ float shared_A[BLOCK_SIZE + 1][BLOCK_SIZE];
    __shared__ float shared_B[BLOCK_SIZE + 1][BLOCK_SIZE];
    ...
    if (i < M && j < N) {
        ...
        for (int w_idx = 0; w_idx < (K + BLOCK_SIZE - 1) / BLOCK_SIZE; w_idx++) {
            ...
            for (int k = 0; k < BLOCK_SIZE && (k_start + k) < K; k++) {
                // Accessing shared_A can cause bank conflict, but accessing shared_B does not
