...
__global__ __launch_bounds__(BM * BN * 32 / (WM * WN)) void vectorized_2d_block_tiling_matmul(
    ...
    const int M, const int N, const int K)
{
    ...
    for (uint bkIdx = 0; bkIdx < K; bkIdx += BK) {
        ...
        for (uint dot_idx = 0; dot_idx < BK; ++dot_idx) {
            ...
            // Load a strip of column of size TM * WMITER = 8 of A into registers
            // Strip is located at [warp_row * WM + w_sub_row * W_SUB_M + lane_row * TM, dot_idx]
            ...
            // Load a strip of row of size TN * WNITER = 8 of B into registers
            // Strip is located at [dot_idx, warp_col * WN + w_sub_col * W_SUB_N + lane_col * TN]
